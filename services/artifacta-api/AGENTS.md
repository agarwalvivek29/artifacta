...
| 2026-09-18 | Browser "upload a new version" (ADR-0024): `POST /artifacts/{slug}/versions` now accepts a multipart form (owner-only, upload-UI on, same-origin), appending v(n+1) via a shared `appendVersion` helper. Gated by new `Artifact.via_upload` — only artifacts created through the upload UI are version-editable from the browser (others → 409); CLI/API versioning is unchanged. Dashboard shows an owner-only "new version" action on those artifacts.                                                                                                                                                                                                                                                               | Vivek Agarwal |
